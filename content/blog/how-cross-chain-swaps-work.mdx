...
**Do I need a wallet on both chains?** Only if the swap actually spans two chains — you need a Solana wallet for any swap starting from Solana, and an EVM wallet as the destination if you're swapping into an EVM chain (or as the source, for an EVM-origin swap).

**How long does it take?** A same-chain swap confirms as fast as the one chain it touches. A cross-chain swap depends on both legs' confirmation times — typically well under a minute, since Relay's solver network delivers from its own inventory rather than waiting on a slow canonical bridge.

**What happens if the destination chain leg fails after the first leg succeeds?** This is exactly the scenario the on-chain re-verification step (above) exists to catch — a swap is never marked complete based on a client-reported transaction hash alone.
